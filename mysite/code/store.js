Vue.component('store', {
	...
	methods: {
		...
		storeLogs(game, logs) {
			...
			if (game && game.teams) {
				this.$bus.$emit('display-teams', game.teams);
			}
		...
		openMenu(type) {
			...
			this.active = type;
			this.$bus.$emit(type, this.game, this.damages, this.heals, this.kills);
		}
	},
	computed: {
	...
	mounted() {
		this._boundImport = (game, logs) => this.storeLogs(game, logs);
		this.$bus.$on('import', this._boundImport);
	},
	beforeDestroy() {
		this.$bus.$off('import', this._boundImport);
	}
});